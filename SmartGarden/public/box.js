// Initialize Firebase
  ...
  var ref = database.ref('Node6');
  ref.on('value', gotData, errData);
  
    function gotData(data){
        console.log(data.val());
        var Node6 = data.val();
        var keys = Object.keys(Node6);
        console.log(keys);
        
        numarray = keys.length;
        for (var i=0; i<keys.length; i++){
            var k = keys[i];
            //var initials = Node6[k].initials;
            //var name = Node6[k].name;
            //console.log(k);
             
            //var keysm = Object.keys(Node6[k]);
            //console.log(keysm);
            var nn = "Node6/" + k;
            var ref = firebase.database().ref(nn);
            ref.orderByChild("time").limitToLast(1).on("child_added", function(snapshot) {
            //var latestdata = snap.child("data").val();
            console.log(k, snapshot.key, snapshot.val().data);
            });
        }
...
var rowSize   = 150;
var colSize   = 150;
var gutter    = 30;     // Spacing between tiles
var numTiles  = numarray;    // Number of tiles to initially populate the grid with
...
function createTile() {
  ...
  var element = $("<div></div>").addClass("tile").html(label + "<hr><br><h5>" + latestdata + "</h5>");
  var lastX   = 0;
